...
const BADGES: Badge[] = [
  ...
  },
  {
    ...
    src: "images/certifications/green-industry-badge-1.png",
    box: [240, 183],
  },
  {
    id: "gmi-badge",
...
export default function QualityBadgesAnimatedPage() {
  ...
  useEffect(() => {
    ...
    const setWidth = () => setClientWidth(window.innerWidth);
    setWidth();
        ...
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-16">
          ...
          <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {BADGES.map((b, index) => (
              <figure
                key={b.id}
                className={`group relative rounded-2xl border border-neutral-200 bg-white shadow-sm hover:shadow-2xl transition-all duration-500 p-4 md:p-6 cursor-pointer transform hover:-translate-y-2 ${
                  isVisible
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 translate-y-10"
                }`}
                ...
                <div
                  ...
                  style={{
                    minHeight: "120px", // ขั้นต่ำ mobile
                    height: "auto",
                  }}>
                  {/* Loading placeholder */}
                  {!loadedImages.has(b.id) && (
                  ...
                  <img
                    ...
                    className={`max-w-full max-h-full object-contain p-2 md:p-4 transition-opacity duration-300 ${
                      loadedImages.has(b.id) ? "opacity-100" : "opacity-0"
                    }`}
                    style={{
                      ...
                      height: "auto",
                      maxWidth: "100%",
                      // compute maxHeight from measured clientWidth (safe for SSR)
                      maxHeight:
                        clientWidth === null
                          ? "180px"
                          : clientWidth < 640
                ...
                <figcaption className="relative mt-3 md:mt-4 text-center">
                  ...
                  {b.desc && (
                    <div className="mt-1 text-xs lg:text-sm text-neutral-500 group-hover:text-neutral-700 transition-colors duration-300 leading-tight">
                      {b.desc}
                    </div>
                  )}
                </figcaption>
