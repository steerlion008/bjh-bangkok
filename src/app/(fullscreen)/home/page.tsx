...
export default function FullscreenHomePage() {
  ...
  // Fetch Inbox data from Facebook Ads API (messaging_first_reply)
  const fetchInboxData = useCallback(async (): Promise<number> => {
    try {
  ...
  // Fetch Leads data (customers with ชื่อ และเบอร์) from database
  ...
  // Fetch Booking data (จำนวนนัดผ่าตัดที่ได้วันนี้) from surgery schedule
  const fetchBookingData = useCallback(async (): Promise<number> => {
    try {
  ...
  // Fetch Revenue data from N_Clinic (today's revenue)
  const fetchRevenueData = useCallback(async (): Promise<number> => {
    try {
  ...
  const fetchAllStats = useCallback(async () => {
    ...
    try {
      const [inbox, leads, booking, revenue] = await Promise.all([
        fetchInboxData(),
        fetchLeadsData(),
        fetchBookingData(),
        fetchRevenueData(),
  ...
  }, [fetchInboxData, fetchLeadsData, fetchBookingData, fetchRevenueData]);
...
>>>>>>> Stashed changes
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 overflow-x-hidden">
      ...
      {/* Content */}
      <div className="relative z-10 px-4 py-4 md:py-6 lg:py-8 max-w-7xl mx-auto">
        ...
        >
          <div className="bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 p-3 lg:p-4 flex items-center justify-between">
            {/* User Avatar & Info */}
            <div className="flex items-center gap-3 lg:gap-4">
              <div className="relative">
                <div className="w-10 h-10 lg:w-14 lg:h-14 rounded-full bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center text-white text-lg lg:text-2xl font-bold shadow-lg">
                  {userData?.name?.charAt(0).toUpperCase() || "A"}
                </div>
                <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 lg:w-4 lg:h-4 bg-green-500 rounded-full border-2 border-slate-900"></div>
              </div>
              <div>
                <h2 className="text-sm md:text-base lg:text-xl font-bold text-white">
                  {userData?.name || "Admin"} 👋
                </h2>
                <p className="text-pink-200 text-xs lg:text-sm">
